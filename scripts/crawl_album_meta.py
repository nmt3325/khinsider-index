...
def load_slugs(path):
    """Read targets from a certified live catalogue, not a legacy index."""
    catalogue = live_data.read_catalogue(path)
    return [(row['slug'], row['title']) for row in catalogue['albums']]
...
def load_done(path):
    """Only validated records written by the standalone live crawler count."""
    return {slug: record['_order'][0]
    ...
    with open(path, encoding='utf-8') as f:
        for line in f:
            ...
            if permanent_only and note not in PERMANENT_NOTES:
                continue
            failed.add(live_data.canonical_slug(slug))
    return failed
...
def main(argv=None):
    ...
    ap.add_argument('--catalogue', '--index', dest='index',
                    default=os.path.join(here, '..', 'catalogue.json'))
    ...
    ap.add_argument('--order', choices=('index', 'hash', 'file'), default='index',
                    help='queue order: catalogue order, deterministic hash, or as given')
    ...
    previous = load_done(args.out)  # Reject foreign/legacy output before appending.
    done = {} if args.refresh else previous
    ...
    def worker(item):
        ...
        if html is not None:
            try:
                ...
                record, note = None, 'songlist parse error: %s' % exc
            except Exception as exc:
                record, note = None, 'parse exception: %s' % exc
        elif status == 'gone':
            record = {'slug': slug, 'data_source': live_data.SOURCE, 'status': 'gone',
                      'http_status': 404, 'crawled_at': observed_at}
            note = 'gone'
        else:
            note = str(status)
        with _lock:
            ...
            if record is not None and record['status'] == 'ok':
                ...
                state['gone' if note == 'gone' else 'fail'] += 1
                failures.write('%s\t%s\t%s\t%s\n' % (slug, note, title, observed_at))
                failures.flush()
